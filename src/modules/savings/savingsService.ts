...
export const savingsService = {
  ...
  async create(
    employeeId: string,
    input: { name: string; targetAmount: number; monthlyAuto?: number; deadline?: Date },
  ) {
  ...
  /** Cron entrypoint — auto-debit each ACTIVE goal with monthlyAuto. */
  async runMonthlyAutoSave(now: Date = new Date()) {
    const goals = await prisma.savingsGoal.findMany({
