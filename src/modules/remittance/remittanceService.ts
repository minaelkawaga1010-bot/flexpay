...
import { Beneficiary, moneyHashService } from '../../services/moneyHashService';
...
export const remittanceService = {
  ...
  async quote(amount: number, currency: string) {
    ...
    const finalRate = rate * (1 - FX_SPREAD);
    const fee = Math.round(amount * FEE_RATE * 100) / 100;
    const recipientAmount = Math.round(amount * finalRate * 100) / 100;
  ...
  async send(employeeId: string, amount: number, currency: string, beneficiary: Beneficiary) {
    ...
    return { transferId, estimatedDelivery, recipientAmount: quote.recipientAmount, fee: quote.fee };
  },
};
